...
dist_sp_mat <- euc_dist(sp_mat)

# Need to reconvert sp_proj back to matrix
head(sp_proj)
po <- Matrix::sparseMatrix(
  dims = c(length(unique(sp_proj[, 1])),
           length(unique(sp_proj[, 1]))),
  i = sp_proj[, 1], j = sp_proj[, 2], x = sp_proj[, 3])

po <- data.matrix(sp_proj)
po <- reshape2::dcast(sp_proj, id1 ~ id2, value.var = colnames(sp_proj)[3])
rownames(po) <- po$id1
po <- po[, !(colnames(po) == "id1")]

po <- tidyr::spread(sp_proj, key = c("id1", "id2"),
                    value = colnames(sp_proj)[3])

##
# CA_res <- CA_cluster(sp_mat)
ward_res <- cluster(sp_mat, method = "ward.D2", n_clust = 4)

dbscan_res <- cluster(sp_proj, method = "dbscan")

gmm_res <- cluster(sp_proj, method = "gmm")
