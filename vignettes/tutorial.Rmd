...
*contingency matrix* a matrix containing the distribution of species across sites. The species are in columns and the sites in rows. This table is the first possible input of the *bioRgeo* functions.
...
*adjacency matrix* a square matrix that contains the occurrences of species within sites. Both sites and species are in rows and columns. In case of a bipartite network, there are no links between the same type of nodes, meaning that the adjacency matrix is filled with a lot of 0. For more details, see [here](https://en.wikipedia.org/wiki/Adjacency_matrix).

*distance matrix* a square matrix containing the distances between each pairs of a set, usually sites. Different metrics to compute the distance can be used. This type of matrix is usually stored as a **dist** object in R.
...
## 2.1. Distance matrix
Most of the methods available in *bioRgeo* package, either clustering or network techniques, rely on the projection of the contingency matrix.

*simil()* function allows projecting the contingency table into a distance matrix using a given similarity metric.
Among the metrics that can be used, only one of them, Bray-Curtis, takes into account the abundances of species. Other similarity metrics deal with presence/absence data only.

If euclidean distances are used to construct the distance matrix, then abundances of species can also be provided.
...
*k* clusters. They can be divided in supervised and unsupervised algorithms. When supervised, the user needs to tell how many groups are expected to be found in the data. If the algorithm used in unsupervised, it will identify on its own how many groups structure the data.
When using supervised algorithms, optimizations algorithms can help the users to decide how many groups should structure the data.

*bioRgeo* contains a function **cluster()** that gathers several classical methods of clustering. Before applying any of the available method, **cluster()** computes a distance matrix based on Euclidean distances between pairs of sites.
Available unsupervised algorithms are the following:

Available supervised algorithms are the following:

They come with optimization procedures to identify the optimal number of clusters. These optimization methods are the following:
globalmax, firstmax, Tibs2001SEmax, firstSEmax or globalSEmax

ward.D", "ward.D2", "single", "complete", "average",
"mcquitty", "median", "centroid", "dbscan", "gmm

The methods can either fall into the supervised or unsupervised category.
They are directly imported from existing packages (cluster, dendextend, fastcluster).

Available methods are the following: ward.D, ward.D2, single, complete, average, mcquitty, median, centroid, dbscan and gmm.

Some of the methods available in *bioRgeo* package follow an agglomerative Hierarchical Clustering.
Each agglomerative clustering starts by assigning each data point to a singleton cluster. It then joins iteratively the two clusters that have the minimal distance among all pairwise distances into a bigger cluster. This step is then repeated until each data point belongs to the same big cluster.
The choice of the agglomerative clustering method affects the calculation of the minimal distance between sets of points. 

The [vignette](https://cran.r-project.org/web/packages/fastcluster/vignettes/fastcluster.pdf) of fastcluster package provides further details about some of the available methods.
...
Quick description of Silhouette.

Quick description of each method.
PAM less sensitive to outliers than Kmeans.
...
knitr::kable(data.frame(
  ...
  Method = c("K-Means", "Partitioning Around Medoids", "Mean-Shift",
             "Density-Based Spatial Clustering of Applications with Noise",
             "Gaussian Mixture Models",
             "Ward.D", "Ward.D2", "single", "complete", "UPGMA",
             "WPGMA", "WPGMC", "UPGMC",
...
# Add Mean Shift algorithm
library(meanShiftR)
po <- meanShift(queryData = sp_mat, trainData = sp_mat, algorithm = "LINEAR",
                alpha = 0, iterations = 100)

pop <- meanShift(queryData = sp_mat, algorithm = "KDTREE",
                 alpha = 0, iterations = 100, parameters = c(10, 100))

msh_res <- data.frame(site = rownames(sp_mat),
                      cluster = po$assignment)

factoextra::fviz_dend(po, cex = 0.5,
                      k = 4, # Cut in four groups
                      palette = "jco")

dend <- as.dendrogram(po)
...
# 3. Comparison of different bioregionalizations
## 3.1. Microbenchmark
...
plot_grid(
  # Plot of Kmeans bioregions
  sp_df %>%
    left_join(km_res, by = "site") %>%
    distinct(site, .keep_all = TRUE) %>%
    ggplot(aes(x, y)) +
    ...
    coord_equal() +
    labs(title = "Kmeans bioregions") +
    theme(panel.background = element_rect(fill = "transparent", colour = NA)),
  
  # Plot of Mean Shift bioregions
  sp_df %>%
    left_join(mshift_res, by = "site") %>%
    distinct(site, .keep_all = TRUE) %>%
    ggplot(aes(x, y)) +
    geom_tile(aes(fill = as.factor(cluster), color = as.factor(cluster)),
              alpha = 0.8, width = 1, height = 1) +
    ...
    scale_fill_brewer("Bioregions", palette = "Paired") +
    coord_equal() +
    labs(title = "Mean Shift bioregions") +
    theme(panel.background = element_rect(fill = "transparent", colour = NA)),
  ...
  # Plot of DIANA bioregions
  sp_df %>%
    left_join(diana_res, by = "site") %>%
    distinct(site, .keep_all = TRUE) %>%
    ggplot(aes(x, y)) +
    geom_tile(aes(fill = as.factor(cluster), color = as.factor(cluster)),
              alpha = 0.8, width = 1, height = 1) +
    ...
    scale_fill_brewer("Bioregions", palette = "Paired") +
    coord_equal() +
    labs(title = "DIANA bioregions") +
    theme(panel.background = element_rect(fill = "transparent", colour = NA)),
  
  # Plot of PAM bioregions
  sp_df %>%
    left_join(pam_res, by = "site") %>%
    distinct(site, .keep_all = TRUE) %>%
    ggplot(aes(x, y)) +
    geom_tile(aes(fill = as.factor(cluster), color = as.factor(cluster)),
              alpha = 0.8, width = 1, height = 1) +
