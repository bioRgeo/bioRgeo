...
*similarity()* function allows projecting the contingency table into a distance matrix using a given similarity metric.
...
```{r projection}
sp_proj <- similarity(sp_mat, similarity = "simpson")

knitr::kable(head(sp_proj))

# With data frame input
sp_proj_df <- similarity(sp_df[which(sp_df$pa != 0), ],
...
sp_proj_w <- similarity(sp_mat_w, similarity = "bray")
...
po <- Matrix::sparseMatrix(
  dims = c(vc, vc), i = el[, 1], j = el[, 2], x = value)
po <- Matrix::sparseMatrix(
  dims = c(vc, vc), i = el[, 1], j = el[, 2], x = value)

# CA_res <- CA_cluster(sp_mat)
ward_res <- cluster(sp_proj, method = "ward.D2", n_clust = 4)
