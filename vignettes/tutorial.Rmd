...
```{r package_scheme, fig.label = "Workflow of the package.", out.width = "70%", echo = FALSE}
...
Since `bioRgeo` relies on two possible inputs, we let the option to users to switch from one format to the other using either `contingency()` or `contingency_to_df()` functions as illustrated in the upper part of the previous figure. 
...
```{r virtual_sp_response_curve, fig.label = "Example of response curve for one virtual species.", out.width = "70%", echo = FALSE}
...
```{r virtual_sp_map, fig.label = "Example of suitability map for one virtual species.", out.width = "70%", echo = FALSE}
...
```{r 100_suitabilities, fig.label = "100 suitability maps for the virtual species.", out.width = "70%", echo = FALSE}
knitr::include_graphics("../figures/100_suitability.jpeg")
```

See [here](http://borisleroy.com/virtualspecies/) for further information on the `virtualspecies` R package. The code to generate the virtual dataset is also provided in the Appendix of this vignette.
...
sp_mat <- contingency(virtual[which(virtual$pa != 0), ], "site", "sp",
                      ab = NULL)
...
sp_mat_w <- contingency(virtual, "site", "sp", abundance.col = "suitab")
...
# With several methods at once
clus_bio <- cluster(
  sp_mat,
  method = c("kmeans", "pam", "gmm", "ward.D", "ward.D2", "single", "complete",
             "average", "mcquitty"), #, "centroid"),
  n_clust = 4)

# meanshift, dbscan and diana algorithms need to be run separately
clus_bio2 <- cluster(sp_mat, method = c("meanshift", "dbscan", "diana"),
                     n_clust = 4)
...
\newpage
The next chunk runs the different network methods on the virtual dataset. Several algorithms to detect communities can be called at once. Only OSLOM should be called separately since it relies upon a different input (a projected dataset output of `simil()` function).  

```{r network}
# Several algorithms at once
net_bio <-  community(
  dat = virtual[which(virtual$pa > 0), ],
  algorithm = c("greedy", "beckett", "louvain", "walktrap", "infomap"),
  weight = FALSE, input = "data.frame", site.col = "site", species.col = "sp",
  N = 10)

# Only sites
net_bio_site <- net_bio %>%
  filter(cat == "site") %>%
  rename(site = node) %>%
  dplyr::select(-cat)
...
# OSLOM
oslom_mod <- community(
  dat = sp_proj[which(sp_proj$simpson > 0), ], algorithm = "oslom", weight = FALSE,
  input = "data.frame", site.col = "site", species.col = "sp", N = 10,
...
cat(paste0("Number of bioregions detected = ",
           length(unique(oslom_mod$module_oslom))))
...
mbm <- microbenchmark(
  ...
  # network methods
  fastgreedy = community(dat = sp_mat, algorithm = "greedy", weight = FALSE),
  beckett = community(dat = sp_mat, algorithm = "beckett", weight = FALSE),
  louvain = community(dat = sp_mat, algorithm = "louvain", weight = FALSE),
  walktrap = community(dat = sp_mat, algorithm = "walktrap", weight = FALSE),
  oslom = community(
    dat = sp_proj[which(sp_proj$simpson> 0), ], algorithm = "oslom", weight = FALSE,
    input = "data.frame", site.col = "site", species.col = "sp", N = 10,
    n_runs = 1, t_param = 0.1, cp_param = 0.5, hr = 0,
    oslom_id1 = "id1", oslom_id2 = "id2", oslom_proj = "simpson"),
  infomap = community(dat = virtual[which(virtual$pa > 0), ], algorithm = "infomap",
...
```{r microbenchmark_plot, fig.width = 5, fig.height = 5}
...
ggplot(mbm_plot, aes(reorder(expr, time, na.rm = TRUE), time)) +
  ...
  labs(title = "50 runs", x = "Methods", y = "Time (log-transformed ms)") +
  theme_classic() +
  theme(panel.border = element_rect(fill = "NA"))
...
# Gather all bioregions in one dataset
bio_maps <- virtual %>%
  distinct(site, .keep_all = TRUE) %>% # remove duplicates per site
  dplyr::select(-suitab, -pa, -opt, -width) %>% # removing useless columns
  left_join(clus_bio, by = "site") %>% # add clustering techniques
  left_join(clus_bio2, by = "site") %>% # add clustering techniques
  left_join(net_bio_site, by = "site") %>% # add network techniques
  left_join(oslom_mod, by = "site") # add oslom

# Gather dataset by bioregionalization method
bio_maps_tidy <- tidyr::gather(bio_maps, bioregion, val,
                               c(contains("cluster"), contains("module")))

# Plot with facetting
ggplot(bio_maps_tidy, aes(x, y)) +
...
```{r comparison, fig.width = 5, fig.height = 5}
# Test of comparison function
comp <- comparison(bio_maps, site = "site", bio_col = c(6:ncol(bio_maps)),
                   output = "both", thres = 8)
lapply(comp, dim)
head(comp[[2]])
...
comp[[2]] %>%
  ...
  ggplot(aes(x, y)) +
  geom_tile(data = virtual[!duplicated(virtual$site), ],
            inherit.aes = FALSE, aes(x, y),
            fill = "grey", color = "grey",
            alpha = 0.8, width = 1, height = 1) +
  geom_tile(aes(fill = as.factor(uni), color = as.factor(uni)),
            width = 1, height = 1) +
  scale_color_brewer("Bioregions", palette = "Set1") +
  scale_fill_brewer("Bioregions", palette = "Set1") +
  coord_equal() +
  labs(title = "Identical bioregions with a threshold of 8 sites") +
  theme_classic() +
  theme(panel.border = element_rect(fill = NA))
```

With a threshold of at least 8 sites, we therefore obtained 8 bioregions for which all the sites were grouped together through the 13 bioregionalizations used.
If we consider a fewer number of bioregionalizations, 3 for example, we are more likely to classify a bigger amount of sites into the unified bioregions.

```{r comparison2, fig.width = 5, fig.height = 5}
# Gather all bioregions in one dataset
bio_maps2 <- virtual %>%
  distinct(site, .keep_all = TRUE) %>% # remove duplicates per site
  dplyr::select(-suitab, -pa, -opt, -width) %>% # removing useless columns
  left_join(clus_bio2[, c("site", "cluster_diana")],
            by = "site") %>% # add diana
  left_join(net_bio_site[, c("site", "module_greedy", "module_walktrap")],
            by = "site") # add fastgreedy and walktrap

# Test of comparison function
comp2 <- comparison(bio_maps2, site = "site", bio_col = c(6:ncol(bio_maps2)),
                    output = "both", thres = 8)

# Plot of fully concordant areas
comp2[[2]] %>%
  left_join(bio_maps2[, c("site", "x", "y")], by = c("id" = "site")) %>%
  distinct(id, .keep_all = TRUE) %>%
  ggplot(aes(x, y)) +
  geom_tile(data = virtual[!duplicated(virtual$site), ],
            inherit.aes = FALSE, aes(x, y),
...
With 3 bioregionalizations only, we can see that almost every pixel can be classed into a common bioregion.

\newpage

We can also plot the concordance across the bioregionalizations for one site only. We here look at the site 103.

```{r comparison_onesite, fig.width = 5, fig.height = 5}
# Plot percentage of concordance for one site only
site103 <- reshape2::melt(comp[[1]])
...
```{r pair_bioregions, fig.width = 5, fig.height = 5}
# Only two bioregionalizations selected
pair_bio <- bio_maps[, c("site", "x", "y", "module_beckett", "module_greedy")]

table(pair_bio$module_beckett, pair_bio$module_greedy)
...
pair_sf <- st_sf(site = pair_bio[, "site"],
                 st_sfc(list_tiles)) %>%
  left_join(pair_bio[, c("site", "module_beckett")], by = "site") %>%
  group_by(module_beckett) %>%
  summarise()

pair_sf2 <- st_sf(site = pair_bio[, "site"],
                  st_sfc(list_tiles)) %>%
  left_join(pair_bio[, c("site", "module_greedy")], by = "site") %>%
  group_by(module_greedy) %>%
  summarise()

plot_grid(
  ggplot(pair_sf) +
    geom_sf(aes(fill = as.factor(module_beckett))) +
    scale_fill_brewer("Beckett", palette = "Paired") +
    theme_classic() +
    theme(panel.border = element_rect(fill = NA)),
  ggplot(pair_sf2) +
    geom_sf(aes(fill = as.factor(module_greedy))) +
...
vmeasure_calc(x = pair_sf, x_name = module_beckett,
              y = pair_sf2, y_name = module_greedy)
```
...
```{r autocorrelation, fig.width = 5, fig.height = 5}
## Autocorrelation
# https://mgimond.github.io/Spatial/spatial-autocorrelation-in-r.html
library(sp)

bio_maps_moran <- bio_maps
# Convert bioregions into numeric
bio_names <- colnames(bio_maps_moran)[6:ncol(bio_maps_moran)]
...
```{r zscore_scheme, fig.label = "Principle of the zscore calculation.", out.width = "70%", echo = FALSE}
...
with $k_i$ the number of sites in its assigned bioregion where species $i$ occurs, $\overline{k_{si}}$ the average number of sites species from the bioregion occur in and $\sigma_{k_{si}}$ the standard deviation.  
...
The next chunk illustrates how `contrib()` function returns these 3 indices for the Infomap bioregionalization.

```{r contrib, fig.width = 5, fig.height = 5}
# Infomap bioregionalization
ggplot(bio_maps_tidy[which(bio_maps_tidy$bioregion == "module_infomap"), ],
       aes(x, y)) +
  geom_tile(aes(fill = as.factor(val), color = as.factor(val)),
            alpha = 0.8, width = 1, height = 1) +
  scale_color_brewer("Bioregions", palette = "Paired") +
  scale_fill_brewer("Bioregions", palette = "Paired") +
  coord_equal() +
  labs(title = "Infomap bioregionalization") +
  theme_classic() +
  theme(panel.border = element_rect(fill = NA))

# Bioregions for sites obtained with Infomap
infomap_site <- net_bio_site[, c("site", "module_infomap")] %>%
  rename(mod_site = module_infomap)
...
```{r contrib_plot, fig.width = 5, fig.height = 5}
...
```{r Cz_space, fig.width = 5, fig.height = 5}
...
scores2 <- contribute(dat = scores_oslom,
                      sp_col = "sp", site_col = "site",
                      bioregion_col = "module_oslom", bioregion_sp_col = NULL)
```
...
```{r interact, fig.width = 5, fig.height = 5}
...
# Manually checking the function
dat <- virtual[which(virtual$pa > 0), ]; input_format = "data.frame"
weight = FALSE; site <- "site"; sp <- "sp"; ab <- NULL
metric = "simpson"; cluster_method <- c("kmeans", "diana")
n_clust = 4;
optim_method = "firstSEmax"; nstart = 25; B = 50; K.max = 20;
network_method <- c("louvain", "greedy");
N = 10; n_runs = 1; t_param = 0.1; cp_param = 0.5; hr = 0;
contribute_algo = NULL; interact_algo = NULL

rm(dat); rm(input_format); rm(weight); rm(site); rm(sp); rm(ab); rm(metric)
rm(cluster_method); rm(n_clust); rm(optim_method); rm(nstart); rm(B); rm(K.max)
rm(network_method); rm(N); rm(n_runs); rm(t_param); rm(cp_param); rm(hr)
rm(contribute_algo); rm(interact_algo)
...
all_res <- all_steps(
  ...
  optim_method = "firstSEmax", n_clust = 4, nstart = 25, B = 50, K.max = 20,
  network_method = c("louvain", "greedy"),
...
# Appendix
## Code to generate virtual dataset
The following chunk shows how the virtual data was built. It is not run when loading the vignette, since the outputs are directly stored with the package.  
