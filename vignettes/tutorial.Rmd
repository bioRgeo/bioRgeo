...
bip_df <- community(dat = sp_df[which(sp_df$pa > 0), ],
                    algo = "greedy",
...
# OSLOM
# next line is not run, the result is directly stored with the package
# (second element of 'virtual' object)
...
# Precise in the .man of community() than some algorithms give a module for both sites and species (infomap, igraph algorithms)
# check cluster_ward
...
mbm <- microbenchmark(
  ...
  infomap = community(dat = sp_df[which(sp_df$pa > 0), ], algo = "infomap",
                      weight = FALSE, input = "data frame", site = "site",
                      sp = "sp", N = 10),
...
```{r microbenchmark_plot, eval = FALSE}
...
We can also plot the concordance across the bioregionalizations for one site only.
...
site103 <- left_join(site103,
                     sp_df[!duplicated(sp_df$site), c("site", "x", "y")],
                     by = "site")

coord103 <- distinct(sp_df[which(sp_df$site == "site103"),
                           c("site", "x", "y")])
...
Function `contrib()` with three indices as output.

The following figure illustrates how the $\rho$ score from [Lenormand et al. (2019)]((https://doi.org/10.1002/ece3.4718)) is calculated.
...
The test value $\rho_{ij}$ is negative if the species $i$ is under‐represented in region $j$, equal to 0 if the species $i$ is present in region $j$ in the   same proportion as in the whole study area, or positive if the species $i$ is over‐represented in region $j$. In  the latter case, we  consider that the species $i$ contribute positively to region $j$ and the level of contribution depends on the $\rho_{ij}$  valu

Formula of `Cz` scores:

$$
C_{i} = 1 - \sum_{i = 1}^{N_m}{(\frac{k_{is}}{k_{i}})^2} 
$$

where $C_i$ is the participation coefficient of node $i$, $k_{is}$ is the number of links of node $i$ to region $s$, $k_i$ is the total number of links of node $i$ and $N_m$ is the total number of regions. We calculated the participation coefficient at each level of the biogeographical structure identified by Map Equation. 
...
scores_infomap <- left_join(sp_df[which(sp_df$pa > 0),
                                  c("site", "sp", "x", "y")], 
...
Some algorithms (like OSLOM) only classify sites into bioregions and do not assign species to bioregions.  

In this case, the argument `bioregion_sp_col` is set to `NULL` and the function `contribute()` assign species to a bioregion based on the most frequent bioregion across the sites where the species occurs. 

```{r contrib_proj_oslom}
# Merging the bioregion identification with long-format data frame
scores_oslom <- left_join(sp_df[which(sp_df$pa > 0),
                                  c("site", "sp", "x", "y")], 
                            oslom_mod, by = "site")
head(scores_oslom)
...
tmp <- left_join(sp_df[which(sp_df$pa > 0), ], oslom_mod, by = "site")
