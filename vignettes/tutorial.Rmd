...
# euc dist details
sp_mat[1:5, 1:5]
dim(Matrix::tcrossprod(sp_mat))

dist_sp_mat <- euc_dist(sp_mat)
dist_sp_mat[upper.tri(dist_sp_mat)] <- NA
diag(dist_sp_mat) <- NA

# Conversion to dataframe
dist_sp_mat <- reshape2::melt(dist_sp_mat, varnames = c("id1", "id2"))
# Remove NAs
dist_sp_mat <- dist_sp_mat[complete.cases(dist_sp_mat), ]
colnames(dist_sp_mat) <- c("id1", "id2", "euclid")

# similarity function
sp_proj <- similarity(sp_mat, similarity = "simpson")
sp_proj <- sp_proj[, c("id1_name", "id2_name", "simpson")]
knitr::kable(head(sp_proj))

# Comparison
po <- left_join(sp_proj, dist_sp_mat, by = c("id1_name" = "id2",
                                             "id2_name" = "id1"))

ggplot(po, aes(simpson, euclid)) +
  geom_point()
...
To project the network, we can use the *similarity()* function.
