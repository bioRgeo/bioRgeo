...
The first element corresponds to the virtual dataset and comes as a `data.frame` with 40000 rows and 9 columns. The second element corresponds to the partition found by the OSLOM algorithm (see below).  
...
# Redo main figure
# Add modularity statistics as an ouput for some algorithms
...
OSLOM uses an iterative process to detect statistically significant communities with respect to a global null model (i.e., random graph without community structure). The main characteristic of OSLOM is that it is based on a score used to quantify the statistical significance of a cluster in the network [(Lancichinetti et al., 2010)](https://doi.org/10.1371/journal.pone.0018961). The score is defined as the probability of finding the cluster in a random null model.

The random null model used, called configuration model, in OSLOM preserves the distribution of the number of neighbors of a node (i.e., the degree distribution).

OSLOM algorithm is non parametric in the sense that it identifies the statistically significant partition, without defining the number of  communities a priori. However, the tolerance value that determines whether a cluster is significant or not might play an important role for the determination of the clusters found by OSLOM.

The original code from [OSLOM webpage](http://www.oslom.org) has been compiled and is available in `bioRgeo`. OSLOM can be run via `network()` function or using the internal function `oslom()`.

The next chunk is not run when loading the vignette, the result being directly stored with the package (second element of `virtual` object). 
...
```{r OSLOM_output}
# OSLOM output available in the package
oslom_mod <- virtual[[2]]
...
```{r bipartite}
# With fastgreedy
bip <- network(dat = sp_mat, algo = "greedy", weight = FALSE)

# fastgreedy, input data frame
bip_df <- network(dat = sp_df[which(sp_df$pa > 0), ],
                  algo = "greedy",
...
bip_beckett <- network(dat = sp_mat, algo = "beckett", weight = FALSE)

# With Louvain
bip_louvain <- network(dat = sp_mat, algo = "louvain", weight = FALSE)

# With Walktrap
bip_walktrap <- network(dat = sp_mat, algo = "walktrap", weight = FALSE)
...
# Merging run_infomap with network
pop <- network(
...
mbm <- microbenchmark(
  ...
  # network methods
  fastgreedy = network(dat = sp_mat, algo = "greedy", weight = FALSE),
  beckett = network(dat = sp_mat, algo = "beckett", weight = FALSE),
  louvain = network(dat = sp_mat, algo = "louvain", weight = FALSE),
  walktrap = network(dat = sp_mat, algo = "walktrap", weight = FALSE),
...
all_bioregions <- sp_df %>%
  ...
  left_join(bip_site_beckett, by = "site") %>% # add Beckett (LPAwb)
  distinct(site, .keep_all = TRUE) %>%
  rename(beckett = module) %>%
  left_join(bip_site_infomap, by = "site") %>% # add infomap
  distinct(site, .keep_all = TRUE) %>%
...
plot_grid(
  ...
  # Plot of Beckett (LPAwb) bioregions
  sp_df %>%
    left_join(bip_site_beckett, by = "site") %>%
    distinct(site, .keep_all = TRUE) %>%
    ggplot(aes(x, y)) +
    geom_tile(aes(fill = as.factor(module), color = as.factor(module)),
              alpha = 0.8, width = 1, height = 1) +
...
all_bioregions_sp$greedy <- as.numeric(as.character(
  all_bioregions_sp$greedy))
all_bioregions_sp$beckett <- as.numeric(as.character(all_bioregions_sp$beckett))
