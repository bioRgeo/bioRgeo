...
bip_site <- bip %>%
  filter(cat == "site") %>%
  rename(site = node) %>%
  dplyr::select(site, module_greedy)

bip_site_beckett <- bip_beckett %>%
  filter(cat == "site") %>%
  rename(site = node) %>%
  dplyr::select(site, module_Beckett)

bip_site_louvain <- bip_louvain %>%
  filter(cat == "site") %>%
  rename(site = node) %>%
  dplyr::select(site, module_louvain)

bip_site_walktrap <- bip_walktrap %>%
  filter(cat == "site") %>%
  rename(site = node) %>%
  dplyr::select(site, module_walktrap)
...
infomap_site <- infomap_mod %>%
  filter(cat == "site") %>%
  rename(site = node) %>%
  dplyr::select(site, module_infomap)
