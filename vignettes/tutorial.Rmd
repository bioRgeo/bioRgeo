...
# Precise in the .man of network() than some algorithms give a module for both sites and species (infomap, igraph algorithms)
# check cluster_ward
...
mbm <- microbenchmark(
  ...
  infomap = network(dat = sp_df[which(sp_df$pa > 0), ], algo = "infomap",
                    weight = FALSE, input = "data frame", site = "site",
                    sp = "sp", N = 10),
...
# Plot
all100 %>%
  left_join(bio_maps[, c("site", "x", "y")], by = c("id1" = "site")) %>%
...
all100 %>%
  ...
  summarise() %>%
  ggplot() +
  geom_sf(aes(color = as.factor(pair_bio)), show.legend = FALSE)
...
where $P_i$ is the participation coefficient of node $i$, $k_{is}$ is the number of links of node $i$ to region $s$, $k_i$ is the total number of links of node $i$ and $N_m$ is the total number of regions. We calculated the participation coefficient at each level of the biogeographical structure identified by Map Equation. 

$$
z_i = \frac{k_i - \overline{k_{si}}}{\sigma_{k_{si}}}
$$

The within-module degree z-score measures how well-connected node $i$ is to
other nodes in the module.

The next chunk illustrates how `contrib()` function returns these 3 indices.

```{r contrib}
tmp <- left_join(sp_df[which(sp_df$pa > 0), ], infomap_site, by = "site")
# table(tmp$sp, tmp$bioregion)

scores <- contribute(dat = tmp, sp_col = "sp", site_col = "site",
                     bioregion_col = "module")
