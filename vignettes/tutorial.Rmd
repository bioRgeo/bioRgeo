...
`bioRgeo` contains a function \code{**cluster()**} that gathers several classical methods of clustering. Some available method are based on a distance matrix. For these methods, you can directly provide the output of `simil()` function. If you feed `cluster()` with a long-format table, `cluster()` will compute a distance matrix based on Simpson dissimilarity metric.  
